...
import { hideContent, isContentHidden } from "../privacy/ContentVisibilityService.js";
...
export function isMemoryHidden(id) {
  return hiddenIds().includes(String(id)) || isContentHidden(id);
}
...
export function getGentleMemory(reference = new Date(), data = readLocalData()) {
  ...
  ].filter((item) => item.date && item.date < today && !isMemoryHidden(item.id));
  const index = deterministicIndex(today, candidates.length);
  return index >= 0 ? candidates[index] : null;
}

export function hideGentleMemory(id) {
  hideContent({ id: String(id), type: "memory", label: "Memória que você escolheu não rever" });
...
export function getSameDayMemory(reference = new Date(), data = readLocalData()) {
  ...
  const candidates = [
    ...
    && Number(item.date.slice(0, 4)) < currentYear && !isMemoryHidden(item.id));
